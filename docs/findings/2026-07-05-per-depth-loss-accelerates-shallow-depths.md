...
Status: confirmed, mechanism retired (pre-registered final-accuracy bar missed, ~1.5σ; pre-registered
...
> **Provenance (2026-09-13):** recovered from branch `claude/nice-hypatia-4m5xso`, which
> was written 2026-07-05 and never opened as a PR — the work was silently abandoned
> (the July branch predates the #143 layout, so its harness commits could not be
> rebased and were dropped; only this record survives). The mechanism it grades, the
> refiner's depth loop, was retired on 2026-09-12
> (`2026-09-12-depth-recurrence-is-suppressed-not-exploited.md`), so this entry is
> kept as knowledge, not as a live lead. #74 closed superseded-by that kill.
