...
Protocol notes, so the number is comparable to something:
  ...
  - No stop-word filter, no candidate restriction. The GPT-2 *paper* numbers
    ...
    (instruments.yardstick.calibrate_gpt2) — see GPT2_SMALL_REFERENCE below.
...
scores our checkpoints (instruments.yardstick.eval_yardstick), the calibration GPT-2, and the
