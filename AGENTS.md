...
All markdown (.md) files should ideally be placed in the `docs/` folder, except for `auxmd.md`, `README.md`, and `AGENTS.md`. The `docs/` folder is used to write ideas worth exploring or annotations for debugging and researching later. Content inside `docs/` must not be deleted, only added to (you can append new content or edit existing text to correct it, but do not remove/delete existing information).≈
